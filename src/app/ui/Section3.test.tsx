...
const getHighlightCellClass = (
  ...
  isHoveredByUser: boolean
) => {
  const baseClass =
    "border border-border-primary bg-bg-primary p-3 transition-all ease-in-out duration-200";
  const hoverClass =
    "hover:bg-amber-50 hover:outline-1 hover:outline-accent-hover outline-transparent";
  const activeClass = "bg-amber-50! outline-1 outline-accent-hover";

  // 사용자가 직접 hover 중이면 기본 hover 스타일만 적용
  if (isHoveredByUser) {
          ...
          <tr>
            ...
            <td
              className={`border border-border-primary p-3 text-end transition-all ease-in-out duration-200 ${
                !isHoveredByUser && activeCell === "swDevCost"
                  ? "bg-amber-50 outline-1 outline-accent-hover"
                  : "bg-bg-secondary hover:bg-amber-50 hover:outline-1 hover:outline-accent-hover outline-transparent"
              }`}
              colSpan={2}
            >
...
// 자동 하이라이트 순환 훅
const useAutoHighlight = (isTouchDevice: boolean) => {
  const [activeCell, setActiveCell] = useState<HighlightCellId | null>(null);
  const [isHoveredByUser, setIsHoveredByUser] = useState(false);
  ...
  const handleMouseEnter = useCallback(() => {
    ...
    setIsHoveredByUser(true);
    stopInterval();
    setActiveCell(null);
  }, [isTouchDevice, stopInterval]);

  const handleMouseLeave = useCallback(() => {
    if (isTouchDevice) return;
    setIsHoveredByUser(false);
    startInterval();
  }, [isTouchDevice, startInterval]);
...
const Section3Auto = () => {
  ...
  const { activeCell, isHoveredByUser, handleMouseEnter, handleMouseLeave } =
    useAutoHighlight(isTouchDevice);
